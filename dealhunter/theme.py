...
This is the shared house theme module. The palettes are the single source of
truth for every app's colours; `apply()` styles a Tk root with them. This app
draws its interface as a web page rather than with Tk, so it takes the same
palettes through `css_vars()` and never calls `apply()` - tkinter is imported
lazily inside the Tk helpers so the packaged build (which excludes tkinter)
...
def css_vars(dark: bool) -> str:
    """The palette as CSS custom properties: --bg, --panel, --field-border..."""
    c = palette(dark)
    return " ".join(f"--{k.replace('_', '-')}: {v};" for k, v in c.items())
